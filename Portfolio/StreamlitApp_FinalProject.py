...
INPUT_FEATURES = [
    "int_rate", "term", "fico_avg", "dti",
    "inq_last_6mths", "payment_to_income",
]
...
def load_pipeline(_session, bucket):
    ...
    s3_client.download_file(Bucket=bucket, Key=f"sklearn-pipeline-deployment/{filename}", Filename=filename)
...
def load_shap_explainer(_session, bucket):
    ...
    if not os.path.exists(local_path):
        s3_client.download_file(Bucket=bucket, Key=f"explainer/{explainer_name}", Filename=local_path)
...
def display_explanation(user_inputs: dict, session, bucket):
    ...
    st.info(f"**Business Insight:** The most influential factor in this prediction was **{label_map.get(top_feature, top_feature)}**.")
